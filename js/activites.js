...
;(function () {
  ...
  function renderList(container) {
    ...
    el.innerHTML = acts.map(function (act) {
      ...
      return '<div data-act="openCard" data-id="' + act.id + '" style="background:' + c.light + ';border-left:4px solid ' + c.bg
        ...
        + '<button data-act="showDetail" data-id="' + act.id + '" type="button" '
        + 'style="padding:.2rem .5rem;border:1px solid ' + c.bg + ';background:#fff;color:' + c.bg
        + ';border-radius:6px;font-size:.68rem;font-weight:600;cursor:pointer">ℹ️ Détail</button>'
        + '</div>'
        + '</div>';
    }).join('');
  }
  ...
  function showCreateModal() {
    ...
    // Phase par défaut (masquée — la phase se gère par élève dans la section dispatch)
    body += '<input type="hidden" id="actPhaseDefault" value="' + (window.curPhase || 'formatif') + '">';
